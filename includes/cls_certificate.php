...
class certificate
{
    ...
    */
    function delete_cert(&$msg){
        ...
        return $GLOBALS['db']->query($delete_sql);
    }
     ...
     */
     function check_oauth_certificate($token){
        ...
        $params['certi_url'] = $base_url;
        $params['certi_session'] = STORE_KEY;
        $params['ac'] = $this->make_shopex_ac($params,$token);
        $rall = $this->oauth->request($token)->post($type,$params,$time);
        $response = $rall->parsed();
        return $response;
     }
    ...
    */
    ...
    /**
    * 功能：检查是否授权
    * @param $token  oauth登录token
    * @return  bool
    */
    function getsninfooauth($token){
        $type = OAUTH_API_PATH.'/online/getsninfooauth';
